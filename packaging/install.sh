...
CONFIG_DIR="/etc/gwarden"
STATE_DIR="/var/lib/gwarden"
LOG_DIR="/var/log/gwarden"

# Detect project root (script should be in packaging/)
...
install_configs() {
    ...
    if [ ! -f "$CONFIG_DIR/ghostnet.toml" ]; then
        if [ -f "$PROJECT_ROOT/examples/ghostnet.toml" ]; then
            install -Dm644 "$PROJECT_ROOT/examples/ghostnet.toml" "$CONFIG_DIR/ghostnet.toml"
            print_success "Installed example topology to $CONFIG_DIR/ghostnet.toml"
        fi
    else
        print_warning "Config already exists at $CONFIG_DIR/ghostnet.toml (keeping existing)"
    fi
    ...
    if [ -d "$PROJECT_ROOT/examples/policies" ]; then
        for policy in "$PROJECT_ROOT/examples/policies"/*.toml; do
            if [ -f "$policy" ]; then
    ...
    chmod 644 "$CONFIG_DIR"/*.toml "$CONFIG_DIR"/*.toml 2>/dev/null || true
    chmod 755 "$STATE_DIR"
    chmod 755 "$LOG_DIR"
}
...
install_completions() {
    ...
    if [ -d "${INSTALL_PREFIX}/share/bash-completion/completions" ]; then
        install -Dm644 "$PROJECT_ROOT/release/completions/gwarden.bash" \
    ...
    if [ -d "${INSTALL_PREFIX}/share/zsh/site-functions" ]; then
        install -Dm644 "$PROJECT_ROOT/release/completions/gwarden.zsh" \
...
install_man_pages() {
    ...
    for manpage in "$PROJECT_ROOT/release/man"/*.1; do
        if [ -f "$manpage" ]; then
...
post_install() {
    ...
    echo -e "   ${BLUE}sudo nano $CONFIG_DIR/ghostnet.toml${NC}"
    echo ""
    ...
    echo "   • Topology: $CONFIG_DIR/ghostnet.toml"
...
Installation Directories:
    ...
    State:       $STATE_DIR
    Logs:        $LOG_DIR
    Completions: ${INSTALL_PREFIX}/share/{bash-completion,zsh}/
