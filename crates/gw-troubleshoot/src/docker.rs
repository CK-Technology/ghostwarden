            ...
            if output.status.success() {
                ...
                for line in addr_info.lines() {
                    if line.contains("inet ") {
                        let ip = line.split_whitespace().nth(1).unwrap_or("unknown");
    ...
    async fn check_network_conflicts(&self) -> anyhow::Result<Vec<DiagnosticResult>> {
        ...
        if output.status.success() {
            ...
            if let Ok(networks) = serde_json::from_str::<serde_json::Value>(&info)
                && let Some(ipam) = networks[0].get("IPAM")
                && let Some(config) = ipam.get("Config")
                && let Some(subnet) = config[0].get("Subnet")
            {
                ...
                if subnet_str.starts_with("10.") || subnet_str.starts_with("192.168.") {
                    results.push(
                        DiagnosticResult::new(
                            ...
                            format!(
                                "Docker subnet {} may overlap with GhostWarden networks",
                                subnet_str
                            ),
                        )
                        .with_suggestion(
                            "Ensure Docker and GhostWarden use non-overlapping subnets",
                        ),
                    );
                }
            }
